...
public class DocumentsDI implements DocumentsDao {
	...
	private static final String SQL_INSERT 			= 
			"INSERT INTO document (agence,client,type_doc,chemin_F,date_ajout) VALUES (?,?,?,?,?)";
	 ...
	 public boolean creerDocuments(Documents documents, AtomicReference<String> errorMsg) {
		  ...
		  if (null!=cn) {
			  try {
				  ...
				  req.setString(5, documents.getDate_ajout());
	...
	public Boolean getDocumentsVerifie(String agence,String client,String type_doc) {
		...
		if (null != cn) {
			try {
				req = cn.prepareStatement("SELECT * FROM document WHERE agence=? AND client=? AND type_doc=?");
				req.setString(1, agence);
				req.setString(2, client);
				req.setString(3, type_doc);
				res = req.executeQuery();
				 while (res.next()) {
					 verifie = true;
				}
	...
	public Boolean getDocumentsVerifieF(String agence,String client,String fichier) {
		...
		if (null != cn) {
			try {
				req = cn.prepareStatement("SELECT * FROM document WHERE agence=? AND client=? AND chemin_F=?");
				req.setString(1, agence);
				req.setString(2, client);
				req.setString(3, fichier);
	...
	private static Documents map(ResultSet res) throws SQLException {
		Documents documents= new Documents();
			...
			documents.setDate_ajout(res.getString("date_ajout"));
