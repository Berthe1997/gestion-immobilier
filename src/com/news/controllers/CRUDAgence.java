		...
		//===================NEW CHAMP=======================================//				
			...
			agence.setSodeci(sodeci);
				...
				if(agenceDI.creerAgence(agence, errorMsg)) {
					request.setAttribute("message", new Message("Agence enregistré avec succès."+ errorMsg.get(), 0, "green"));
				}else request.setAttribute("message", new Message("Echec enregistrement Agence. " + errorMsg.get(), 0, "red") );
			}else request.setAttribute("message", new Message("Cet Agence existe déjà. " + errorMsg.get(), 0, "red") );
		}
