     ...
     <!-----------------------------------------------FIN DE L'ENTÊTE------------------------------------------  -->
      ...
      <div class="card shadow mb-4">
        <div class="card-header py-3">
          <h6 class="m-0 font-weight-bold text-primary" style="text-align:center;">DETAIL PAIEMENT LOYER    <a class="btn btn-danger float-left" href="<c:url value="/menus?menu=paiement_loyer"/>"> Retour</a></h6>
        </div>
        <div class="card-body">
            ...
            <div class="col-sm-12 mb-10 mb-sm-0"> 
              ...
              <c:forEach items="${listPaiL}" var="mapPaiL" varStatus="boucle">
		                <tr>              
		                  ...
		                  <td>${mapPaiL.type}</td>
		                  <td>${mapPaiL.montant}</td> 		              
		                  <td>${mapPaiL.date}</td> 		                 		                                                                     
		                  <td align="center">		                 
		                   <a target="_BLANK" href="<c:url value="/Recu_paiement?id=${mapPaiL.id}&matE=${mapPaiL.matricule}&ans=${mapPaiL.annee}&mois=${mapPaiL.mois}&montantS=${mapPaiL.montant}&code=${mapPaiL.code}"/>" style="border:1px solid grey;border-radius:3px;padding:4px;color:#000;"><i class="fa fa-print fa-lg"></i></a>
							 <c:if test="${sessionScope.rolePr.supp_PL==1}">
							<button onclick="document.location.href='<c:url value="/view_paiement_loyer?idl=${mapPaiL.id}&id=${locataire.id}&matricule=${mapPaiL.matricule}&ans=${mapPaiL.annee}&mois=${mapPaiL.mois}&montantS=${mapPaiL.montant}&type=${mapPaiL.type}&date=${mapPaiL.date}&heure=${mapPaiL.heure}&soldeP=${mapPaiL.montantP}&soldeA=${mapPaiL.montantA}&soldeI=${mapPaiL.montantI}&page=${page}&crud=PaiL" />'">
